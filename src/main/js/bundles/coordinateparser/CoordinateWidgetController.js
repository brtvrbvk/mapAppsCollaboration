...
define([
        ...
        "dojo/number",
        "dojo/string",
        "agiv_bundles/base/search/SearchTopics",
        "dojo/_base/declare",
        "ct/Stateful",
        "ct/_Connect",
    ...
    function (
        ...
        return declare([
            ...
            ],
            {
                ...
                _isCoordinate: function (val) {
                    if (!this.coordinateParsers) {
                        return {isNaN: true};
                    }
                    var coordinate = null;
                    d_array.some(this.coordinateParsers, function (parser) {
                        coordinate = parser.isCoordinate(val);
                        if (!coordinate.isNaN) {
                            return true;
                        }
                        return false;
                ...
                isValidCoordinatePair: function (val) {
                    var isValid = false;
                    d_array.some(this.coordinateParsers, function (parser) {
                        isValid = parser.isValidCoordinatePair(val);
                        if (isValid) {
                            return true;
                        }
                    }, this);
                ...
                _onValueChange: function (evt) {
                    ...
                    var e = evt._evt,
                        ...
                        coord = this._isCoordinate(val);
                    if (coord && !coord.isNaN && coord.x && coord.y) {
                        this._goToFirstResult = true;
                        if (e.keyCode === d_keys.ENTER) {
                            //hightlight the coordinate
                            this._broadCast(SearchTopics.HIGHLIGHT_COORDINATE, {
                                result: {
