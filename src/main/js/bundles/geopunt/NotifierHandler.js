define([
    ...
    function (
        ...
        _moduleRoot.NotifierHandler = declare(
            [Stateful],
           
            {
           
                constructor: function () {
                    document.bart_notifier=this;
                ...
                notify: function (message, title, options, image) {
                        ...
                        if(options.xClose ){
                            this.config.box_styles.cursor="default";
                        ...
                        notification.onmouseout = function() {
                            document.bart_notifier.apply_styles(this, document.bart_notifier.config.box_styles);
                        };
                        ...
                        /*
                        notification.isLink=options.isLink;
                        if(options.isLink)
                            notification.onclick = function() {
                                window.open(this.isLink, '_blank');
                            };
                        */
                        var icon = document.createElement('img');
                        icon.src = image;
                        this.apply_styles(icon, this.config.icon_styles);
                        ...
                        this.apply_styles(text, this.config.text_styles);

                        notification.appendChild(text);
                        if (title) {
                            var title_text = document.createElement('div');
                            this.apply_styles(title_text, this.config.title_styles);
...
/*
                        if (message) {
                            ...
                            if(!options.isLink)
                                ...
                                link.setAttribute('href', options.isLink);
                                link.setAttribute('target', '_blank');
                                var txt = document.createTextNode(message);
                                link.appendChild(txt)
                                message_text.appendChild(link);
                            }
...
*/                        
                        if (message) {
                            ...
                            for(var i=0;i<message.messages.length;i++){
                                if(message.messages[i].link){
                                    var link = document.createElement('a');
                                    link.setAttribute('href', message.messages[i].link);
                                    link.setAttribute('target', '_blank');
                                    var txt = document.createTextNode(message.messages[i].text);
                                    link.appendChild(txt)
                                    message_text.appendChild(link);
                                }else{
                                    message_text.appendChild(document.createTextNode(message.messages[i].text));
                                }
                            }
                            text.appendChild(message_text);
                        }
 ...
 */
        ...
        return _moduleRoot.NotifierHandler;
    });